<script lang="ts">
    ...
    let { type, loading }: SubmitButtonProps = $props();
</script>

{#if type == 'default'}
    <button type="submit" class="py-2 px-4 bg-blue hover:bg-opacity-85 transition-all ease-in-out font-bold text-white w-full rounded-lg drop-shadow-sm select-none">
        Se connecter
    </button>
{:else if type == 'cas'}
